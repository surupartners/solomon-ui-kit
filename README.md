...
```
"dependencies": {
  "solomon-ui-kit": "https://hbrown@bitbucket.org/hbrown/npm-test.git#1.3.3",
},
```
...
```
npm install https://hbrown@bitbucket.org/hbrown/npm-test.git#1.3.3 --save
```
...
1. Run `npm run start-release` to automatically bump the package version numbers
   and start a `release/x.y.z` branch (e.g. `release/1.3.3`). You should now be
