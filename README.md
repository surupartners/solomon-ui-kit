...
```
"dependencies": {
  "solomon-ui-kit": "https://hbrown@bitbucket.org/hbrown/npm-test.git#1.4.0",
},
```
...
```
npm install https://hbrown@bitbucket.org/hbrown/npm-test.git#1.4.0 --save
```
...
1. Run `npm run start-release` to automatically bump the package version numbers
   and start a `release/x.y.z` branch (e.g. `release/1.4.0`). You should now be
